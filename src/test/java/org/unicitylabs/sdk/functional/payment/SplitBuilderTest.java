...
public class SplitBuilderTest {
  ...
  public void verifyFailsWhenBurnTokenVerificationFails() {
    ...
    CborDeserializer.CborTag reasonTokenTag = CborDeserializer.decodeTag(reasonData.get(0));
    List<byte[]> reasonTokenData = CborDeserializer.decodeArray(reasonTokenTag.getData());
    List<byte[]> transactions = CborDeserializer.decodeArray(reasonTokenData.get(2));
    ...
    transfer.set(3, Address.fromBytes(invalidRecipient).toCbor());
    ...
    reasonTokenData.set(2, encodeArray(transactions));
    reasonData.set(0, CborSerializer.encodeTag(reasonTokenTag.getTag(), encodeArray(reasonTokenData)));
  ...
  private Token withPayload(Token token, byte[] payload) {
    ...
    List<byte[]> aux = CborDeserializer.decodeArray(mint.get(3));

    aux.set(1, CborSerializer.encodeByteString(payload));
    mint.set(3, encodeArray(aux));
